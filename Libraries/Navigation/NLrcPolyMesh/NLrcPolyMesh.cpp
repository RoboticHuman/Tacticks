...
vector<PassObject*> NLrcPolyMesh::getData(string request, vector<PassObject*>)
{
	if(request=="getRandomPosition"){
		return {new PassObjectVec3(getRandomPosition())};
	}
	return {};
}
...
glm::vec3 NLrcPolyMesh::getRandomPosition()
{
	int nvp;
	unsigned short* p;
	int j;
	do {
		int randomIndex = rand()%(data->npolys);
		nvp = dynamic_cast<PassObjectInt*>(args[0])->getValue();
		p = &(data->polys[randomIndex*nvp*2]);
		j = rand()%nvp;
	} while(p[j] == RC_MESH_NULL_IDX);

	const unsigned short* v = &(data->verts[p[j]*3]);

	const float* orig = data->bmin;
	const float x = orig[0] + v[0]*data->cs;
	const float y = orig[1] + (v[1]+1)*data->ch;
	const float z = orig[2] + v[2]*data->cs;
